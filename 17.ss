    ...
    (cond
      ...
      (cond         
        ...
        (cond
          ...
          [(and (andmap (lambda (x) (if (list? x) (eqv? (1st x) 'ref) (symbol? x))) (2nd datum))
                (ormap list? (2nd datum)))
          (ref-exp (2nd datum) (map parse-exp (cddr datum)))]
...
;        (let ((pos (list-find-position sym syms)))
;          (if (number? pos)
;            (set-car! (list-tail vals pos) value)
;            (apply-env-set! env sym value fail)))]
;      [recursively-extended-env-record
;        (names idss bodiess old-env)
;        (eopl:error 'apply-env-set! "Should not encounter recursively-extended-env-record")])))

;the 'define-exp' would be messed up if we did this

;(define apply-env-define
;  (lambda (sym value)
;    (cases environment global-env
;      [extended-env-record (syms vals env)
;        (begin (set-cdr! syms (append (cdr syms) (list sym)))
;               (set-cdr! vals (append (cdr vals) (list value))))]
;      [else (eopl:error 'apply-env-set! "Global env should not be in this case")])))
...
(define let-loop
  (lambda (vars body)
    (if (null? vars)
      (let-ref-exp '() body)
      (let-ref-exp (list (car vars)) (list (syntax-expand (let-exp (cdr vars) body)))))))
    ...
    (cases expression exp
      ...
      [app-exp (rator rands)
        (let ([proc-value (eval-exp rator env)])
          (cases proc-val proc-value
            [closure-ref (ids bodies env)
              (let ([args (eval-rands-ref rands env ids)])
      ...
      [set!-exp (vars body)
        (if (not (expression? (eval-exp (var-exp vars) env)))
          ...
          (eval-exp (syntax-expand body) env))
          (set-box! (apply-env-ref (if (equal? (eval-exp (var-exp vars) env) (var-exp (2nd (eval-exp (var-exp vars) env))))
                                      (cadddr env)
                                      env)
                                   (2nd (eval-exp (var-exp vars) env))
                                   (lambda (x) x)
                                   (lambda () (eopl:error 'apply-env "variable not found" vars)))
                    (let ([result (let ([current (eval-exp (syntax-expand body) env)])
                                        (if (expression? current) (eval-exp current env) current))])
                            (if (box? result) (unbox result) result))))]
        ;(apply-env-ref env vars (eval-exp (syntax-expand body) env) (lambda () (eopl:error 'apply-env "variable not found" vars)))]
      [define-exp (vars body)
        ;(let ([value (eval-exp (syntax-expand body) env)])
        ;  (apply-env-set!
        ;    env
        ;    vars
        ;    value
        ;    (lambda () (apply-env-define vars value))))]
  ...
  (lambda (rands env vars)
    (if (null? vars)
      ...
      (if (list? (1st vars))
        ...
        (cons (apply-env-ref env (2nd (1st rands)) (lambda (x) x) (lambda () (eopl:error 'apply-env "variable not found" vars)))
...
(define apply-proc
  (lambda (proc-value args)
    (cases proc-val proc-value
      ...
      [closure-ref (ids bodies env)
        (eval-bodies bodies (extend-env (map (lambda (x) (if (list? x)
                                                             (2nd x)
                                                             x)) ids) args env))]
...
;global-env should probably have all the prim-procs to start
;we didn't change init-env but changed the global-env to empty.
...
;if the global-env is empty, add what should be added to the global
...
;        [(=) (= (1st args) (2nd args))]
        ...
        [(list?) (apply list? args)]
        [(pair?) (apply pair? args)];missing "apply" so when doing the apply-proc it doesn't apply pair to all the args which why we didn't pass the last text case for 
                                    ;named-let and define
